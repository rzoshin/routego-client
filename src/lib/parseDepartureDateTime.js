/**
 * Build a local Date from ticket/booking departure fields.
 * Handles YYYY-MM-DD (form input), ISO strings (MongoDB), and legacy display formats.
 */
export function parseDepartureDateTime(departureDate, departureTime) {
  if (!departureDate) return null;

  const rawDate = String(departureDate).trim();
  const rawTime = String(departureTime || "00:00").trim();

  // MongoDB / JSON ISO datetime — use date portion only, then apply time locally
  ...
  // Fallback for strings like "Jun 24, 2026 08:30 AM"
...
export function isDeparturePassed(departureDate, departureTime) {
